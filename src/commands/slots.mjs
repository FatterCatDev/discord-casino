...
async function inCasinoCategory(interaction, kittenMode) {
  ...
  try {
    if (!interaction.guild) {
      return {
        ok: false,
        reason: say('❌ Spin with me inside a server, Kitten.', '❌ Slots can only be played inside a server channel.')
      };
    }
    ...
    if (!catId || catId !== casino_category_id) {
      ...
      return {
        ok: false,
        reason: say(
          `❌ Bring me into ${categoryLabel} before we spin, Kitten.${kittenHint}`,
